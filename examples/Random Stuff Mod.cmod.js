...
EventHandler.bindEvent('World.Generate', function(isFlat, chunkGenerator){
    if (isFlat==true) {
        return;
    }
    chunkGenerator.addGenerateRule(function(biome,level, x,y,z, cx,cz) {
        if (y == level + 1) {
            id=null
            if (biome == 0 && (cx * 4 + x) % (chunkGenerator.randNext(2) + 1) == 0 && (cz * 4 + z) % (chunkGenerator.randNext(2) + 1) == 0) {
                n=chunkGenerator.flowersNoise((cx * 4 + x) / 24, (cz * 4 + z) / 24)
            ...
            if (biome == 1 && (cx * 4 + x) %  (chunkGenerator.randNext(2) + 1) == 0 && (cz * 4 + z) % (chunkGenerator.randNext(2) + 1) == 0) {
