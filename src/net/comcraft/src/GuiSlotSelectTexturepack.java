...
public class GuiSlotSelectTexturepack extends GuiSlot {
    ...
    protected void drawSlot(int i) {
        ...
        if (texturePack.isTexturePackSupported()) {
            ...
            drawStringWithShadow(cc.g, texturePack.getTexturePackName(), (Comcraft.screenWidth - slotWidth) / 2 + 10, y + interval, Graphics.TOP | Graphics.LEFT);

            cc.g.setColor(200, 200, 200);
            drawStringWithShadow(cc.g, texturePack.getTexturePackDescription(), (Comcraft.screenWidth - slotWidth) / 2 + 15, y + interval * 2 + cc.g.getFont().getHeight(), Graphics.TOP | Graphics.LEFT);
        } else if (texturePack.isThereAnyError()) {
            cc.g.setColor(110, 110, 110);
            drawStringWithShadow(cc.g, texturePack.getTexturePackName(), (Comcraft.screenWidth - slotWidth) / 2 + 10, y + interval, Graphics.TOP | Graphics.LEFT);

            cc.g.setColor(0x8B0000);
            drawStringWithShadow(cc.g, texturePack.getErrorMessage(), (Comcraft.screenWidth - slotWidth) / 2 + 15, y + interval * 2 + cc.g.getFont().getHeight(), Graphics.TOP | Graphics.LEFT);
        } else {
            cc.g.setColor(110, 110, 110);
            drawStringWithShadow(cc.g, texturePack.getTexturePackName(), (Comcraft.screenWidth - slotWidth) / 2 + 10, y + interval, Graphics.TOP | Graphics.LEFT);

            cc.g.setColor(0x8B0000);
            drawStringWithShadow(cc.g, cc.langBundle.getText("GuiSlotSelectTexturepack.invalidResolutionText"), (Comcraft.screenWidth - slotWidth) / 2 + 15, y + interval * 2 + cc.g.getFont().getHeight(), Graphics.TOP | Graphics.LEFT);
        }
    }
}
